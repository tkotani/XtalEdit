     ...
     .  eula,nclass)
ctakao sopts is not effective now...
...
c            j = 0
c            if (.not. a2bin(sopts(j1+2:),range,4,0,' ',j,j2-j1-1))
c     .        goto 999
c          elseif (sopts(j1:j1) .eq. 'i')  then
c            if (sopts(j1+1:j1+1) .eq. '=') then
c              j = 0
c              if (.not. a2bin(sopts(j1+2:),lstyle,2,0,' ',j,j2-j1-1))
c     .        goto 999
c            endif
c            j1 = j2+2
c            call nwordg(sopts,0,dc//' ',1,j1,j2)
c            if (j2 .lt. j1) goto 999
c            call defi(olstc, nclass)
c            call clist(lstyle,sopts(j1:),dclabl,z,nclass,nlstc,
c     .        w(olstc))
c            call awrit2('%N shoshl: pairs %n:1i',' ',scrwid,i1mach(2),
c     .        nlstc,w(olstc))
c            goto 42
c          else
c            goto 999
...
c          elseif (sopts(j1:j1+1) .eq. 'r=')  then
c            m = 0
c            j = parg('r=',4,sopts(j1:),m,len(sopts(j1:)),
c     .        dc//' ',1,1,iv,range)
c            if (j .le. 0) goto 999
c
cC         Site list
c          elseif (sopts(j1:j1+4) .eq. 'sites') then
c            if (sopts(j1+5:j1+5) .eq. '=') sopts(j1+5:j1+5) = dc
c            call baslst(10,sopts(j1+5:),ips,nbas,slabl,zz,0,' ',xx,
c     .        nsites,w(oslist))
c
cC         Bond list
c          elseif (sopts(j1:j1+4) .eq. 'bonds') then
c            if (sopts(j1+5:j1+5) .eq. '=') sopts(j1+5:j1+5) = dc
c            call baslst(10,sopts(j1+5:),ips,nbas,slabl,zz,0,' ',xx,
c     .        nbonds,w(oblist))
c          endif
