...
C     implicit none
      ...
      double precision numtok,numstk(0:32),machep,xx
...
C ... the operation
      if ((t2.eq.0).and.(numnum.eq.0)) goto 10900
      if ((t2.eq.8).and.(numnum.lt.3)) goto 10900
      if ((t2.gt.0) .and. (t2.lt.7) .and. (numnum.lt.2)) goto 10900
...
C#ifdefC HANSMR
C  119 continue
C      if (v0 .lt. 0D0) stop 'A2BIN: out of domain in HNSM.'
C      rsm = 0
...
C     implicit none
      logical   resL(0:1)
      integer   resI(0:1),cast,count
      real      resR(0:1)
      double precision  resD(0:1),val
...
C     implicit none
      ...
      logical   lskp
      character*1       t,strn(0:1),term
      double precision res,sign
...
C --- Error handling ---
   99 continue
C#ifdefC APOLLO_BUG
C      if (iprint() .ge. 60) print 332, (strn(i), i=0, j-1)
C#else
...
C -------------------------------------------------------------------
C     implicit none
      character*1 term,str(0:1)
      integer   j,jmax,namlen,maxsiz,parenc,nunop
      integer   opnum,numnum,toktyp,ptktyp,prevop,opstk(0:32),optok
