...
C Remarks
Cu Updates
Cu   02 Feb 01   corrected case when last expr ends at lstr w/out term
C ----------------------------------------------------------------------
C     implicit none
C Passed Parameters
      integer ip,lstr,cast,nsep,nvec,itrm,ix(1)
      character str*(*), sep(nsep)*1
...
C         Still ok if expr runs to last char in string with no term
          ip0 = ip
          if (.not. a2bin(str,res,cast,iv-1,sep(ix(iv)),ip0,lstr-1))
     .      return
        endif
        if (.not. a2bin(str,res,cast,iv-1,sep(ix(iv)),ip,lstr-1)) then
C ...     Try for nam=expr
          if (nvec .lt. 0) then
