...
Cr For z, j, p, a, f, o and b the general syntax is:
Cr   %[n1]x, with x=z, p, a, f, b
Cr   n1 repeats (f, b)
...
C#endif
     ...
     .  (iv(5),fw)
      ...
      parameter (lens=1024)
      character*(lens) s,ss,fchr*28,fm*20,cc*1,ccond*1
      data fchr /' :;,#irdegGltapfbzDwWcox?jNF'/
...
C  ...  Any non-% character
        if (fmt(iff:iff) .ne. '%' .or. fmt(iff:iff+1) .eq. '%%') then
...
C        endif
        if (ia.eq.1)
     .    call bin2av(fm,fw,nblk,ndec,a1,cast,0,i2,' ',mxl,s,ip)
        if (ia.eq.2)
     .    call bin2av(fm,fw,nblk,ndec,a2,cast,0,i2,' ',mxl,s,ip)
        if (ia.eq.3)
     .    call bin2av(fm,fw,nblk,ndec,a3,cast,0,i2,' ',mxl,s,ip)
        if (ia.eq.4)
     .    call bin2av(fm,fw,nblk,ndec,a4,cast,0,i2,' ',mxl,s,ip)
        if (ia.eq.5)
     .    call bin2av(fm,fw,nblk,ndec,a5,cast,0,i2,' ',mxl,s,ip)
        if (ia.eq.6)
     .    call bin2av(fm,fw,nblk,ndec,a6,cast,0,i2,' ',mxl,s,ip)
        if (ia.eq.7)
     .    call bin2av(fm,fw,nblk,ndec,a7,cast,0,i2,' ',mxl,s,ip)
        if (ia.eq.8)
     .    call bin2av(fm,fw,nblk,ndec,a8,cast,0,i2,' ',mxl,s,ip)
...
C   30   continue
C        if (nstrn*nmxstr .lt. ip) write(ia,333) s((nstrn*nmxstr+1):ip)
C      endif
C#else
      if (ifi .ne. 0) write(ia,333) s(1:ip)
C#endif
  333 format(a)
      if (ifi .le. 0) sout = s(1:ip)
      awrite = ip
...
*     call shosyv(0,0,0,6)
      ...
      subroutine bin2av(fmt,w,nblk,ndec,res,cast,i1,i2,sep,mxln,outs,ip)
