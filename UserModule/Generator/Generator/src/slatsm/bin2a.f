...
Ci   ip:    starting position in outstr for write
Co Outputs:
Co   ip:    position of final character written to outstr
Cr Remarks
...
Cr     the number of digits after the decimal point for 'd' format
Cr     the relative precision for 'e' format
Cr     the field width n in fortran format fn.m for 'D' format
...
C Local Variables
     ...
     .  ix(4),iv(4),a2vec,p,isw,isw0,getdig,m,r,ndig,ndige
      parameter (lsmx=80)
...
C --- Convert binary to ascii representation (log, int, char) ---
   ...
   11 strn2 = fmt
      goto 15
   12 call bin2ai('(I16)',res,count,strn2)
      goto 15
...
C   ... Get precsn (or field width for D or F), in iv(1)
        j = j+1
        np = a2vec(lfmt,20,j,2,': ',2,2,2,ix,iv)
        isw = 1 + isw0
...
C --- Copy to outstr ---
   50 continue
      ...
      subroutine bin2ai(fmt,res,count,strn)
      character*(*) fmt, strn
      integer count
      integer res(0:1)
      write(strn,fmt) res(count)
