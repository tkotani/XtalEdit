...
Ci   xtol   tolerance in position change for convergence  (see isw)
Ci   gtol   tolerance in gradient for convergence  (see isw)
...
C     implicit none
     ...
     .  hess(n,n)
C Local variables
      integer nx,ng,ns,nd,nh,nh2,nhs
...
C     double precision wk(n)
      integer i,j,i1mach,isw1,isw32,isw2,isw31,scrwid,chkhss
      parameter (scrwid=80)
...
C#ifdef AWRITE
        if (ipr .ge. 30) then
          j = isw1
          call awrit6(' brmin: start'//
     .      '%?#(n==1|n==2|n==3)#  xtol=%1;2g#%j#'//
     .      '%?#(n==0|n==2|n==3)#  gtol=%1;2g#%j#'//
     .      '  isw=%i  |g|=%1;3g',' ',80,i1mach(2),
     .      j,xtol,j,gtol,isw,dsqrt(ddot(n,g,1,g,1)))
        endif
C#endif
...
C#ifdef AWRITE
        if (scale .ne. 1) then
          call awrit5(' brmin: iter %i  max shift=%1;4g  '//
     .    '|g|=%1;4g  max g=%1;4g  scale H by %1;3g',
     .    ' ',80,i1mach(2),iabs(ir),diff,dsqrt(ddot(n,g,1,g,1)),
     .    gmax,scale)
        else
          call awrit6(' brmin: iter %i  max shift=%1;4g  '//
     .    '|g|=%1;4g  max g=%1;4g%?#n#  evmin=%1;2g#%j#',
     .    ' ',80,i1mach(2),iabs(ir),diff,dsqrt(ddot(n,g,1,g,1)),
     .    gmax,isw31,evmin)
        endif
C#endif
        if (ipr .ge. 60) then
          print '('' Suggested new x: '',20(6f10.6:/18x))',
     .      (x(i), i=1,n)
        endif
      endif
      ...
      j = ir
      if (gmax .eq. 0 .or.
     .  gmax.lt.gtol .and. diff.lt.xtol .and. isw1.eq.2 .or.
     .  gmax .lt. gtol .and. (isw1.eq.0 .or. isw1.eq.3)  .or.
     .  diff .lt. xtol .and. (isw1.eq.1 .or. isw1.eq.3)  .or.
     .  gmax .lt. gtol .and. (isw1.eq.0 .or. isw1.eq.3)) ir = 0
C#ifdef AWRITE
      if (ipr .ge. 20 .and. ir .eq. 0) then
        call awrit4('%x brmin converged to dxmax=%1;2,2g, '//
     .    'gmax=%1;2,2g, |grad|=%1;2,2g in %i iter',' ',scrwid,
     .    i1mach(2),diff,gmax,dsqrt(ddot(n,g,1,g,1)),iabs(j))
      endif
C#endif
      end
