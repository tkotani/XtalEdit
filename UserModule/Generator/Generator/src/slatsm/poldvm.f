...
Ci    lrat: use rational function interpolation
Ci    tol: return when estimated value of next term is less than tol
Co Outputs
Co    yp: interpolated value of y at each x
Co    lerr: (lrat only) points to first point which rat int had problems
C ----------------------------------------------------------------
C     implicit none
      integer np,n
      logical lrat
...
C Local variables
      integer i,m,nmax,ns,ii,ip,i0,iipm,lerr
      parameter (nmax=50)
      double precision den,ho,hp,w,dd,t,c(nmax),d(nmax),
     .  tiny,d1mach
