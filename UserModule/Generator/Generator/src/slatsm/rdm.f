...
C --- Binary read ---
      if (mod(sw,10) .eq. 2 .or. mod(sw,10) .eq. 3) then
        if (mod(sw,10) .eq. 2) then
...
C   ... Matrix is specified real or complex
        ...
        if (i .eq. 0 .and. nr .ne. 0 .and. nr .ne. nint(s(1))) goto 196
        if (i .eq. 1 .and. nc .ne. 0 .and. nc .ne. nint(s(1))) goto 196
        if (i .eq. 0 .and. nr .eq. 0) nr = nint(s(1))
        if (i .eq. 1 .and. nc .eq. 0) nc = nint(s(1))
        goto 20
      else
...
C       print *, 'nelt,i',nelt,i, ' ir,ic,ioff=',ir,ic,ioff,s(i)
        nelt = nelt+1
        ic = ic+1
        if (ltrns) then
          ic = ic-1
...
C --- Force matrix symmetric or hermetian if so stated ---
