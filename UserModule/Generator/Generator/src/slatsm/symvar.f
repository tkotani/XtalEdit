...
C#ifdefC unix
C      call skipbl(recrd,size,i0)
CC ... This branch writes echo `...` to file tmpdir, reads file contents
CC     and parses result
C      if (recrd(i0) .eq. '`') then
C        a = 'echo `'
C        k = 1
C        call strcop(a(7:),recrd(i0+1),min(size-i0,len(a)),'`',k)
C        if (a(k+6:k+6) .ne. '`') goto 999
C        tmpdir = ' '
...
C        i0 = i0+k+1
C      else
C        lrd = a2bin(recrd,dum,4,0,' ',i0,-1)
C      endif
C#else
