...
C$    integer mp_my_threadnum
      ...
      save
      data nn/0/, ii/0/, level/0/, init/0/, ltr/0/, lpr/0/

      stdo = lgunit(1)
      if (ltrace .gt. 0) ltr = ltrace
...
c ... do real operations here
  91  continue
      if (job .eq. 1) then
        ...
        it1(ii) = it
        icall(level-1) = ii
        if (ltr .gt. levx .and. iprint() .gt. 0)
     .    write(stdo,100) 0.001*(it-jt0),str1
  100   format(' >>',f10.2,'   enter ',a)
C       jt1 = it
        return
      else
        it = 1000*cpusec()
        ...
        if (ltr.gt.levx .and. iprint() .gt. 0) then
          if (iwk0(ii).eq.iwk1(ii)) then
            write(stdo,101) 0.001d0*(it-jt0),str2,tm
  101   format(' >>',f10.2,'   exit  ',a,1x,f8.2)
          else
            write(stdo,102) 0.001*(it-jt0),str2,tm,iwk1(ii)-iwk0(ii)
  102       format(' >>',f10.2,'   exit  ',a,1x,f8.2,
  ...
  800 format(/
     .'  ==== workspace ====     calls      == cpu time ===   depth',i2/
     .'  entry   mxin  mxout                per call  total')

      it = 1000*cpusec()
      tm = (it-jt0)/1000d0
      call wquery(mfree,mnow,mmax)
      iwmax = 0.001*mmax+0.5
C     iwnow = 0.001*mnow+0.5
      if (iprint() .gt. 0)
     .  write (ifi,700) 0,0,iwmax,1,tm,tm,'main'
      lev(nn+1)=-1
