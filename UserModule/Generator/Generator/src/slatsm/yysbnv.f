C#define F90
C#ifndefC F90
C      subroutine yysbnv(mode,apr,api,ldap,ija,offs,nlev,
C     .  nn1,nn2,w,ldw,ar,ai,lda,ierr)
C#else
...
Cb Bugs
Cb   yysbnv has not been tested for the hermetian case, 
...
C#endif
      ...
      call isanrg(mode1,0,1,'yysbnv:','10s digit mode',.true.)
      modl = 10*mod(mode0,3)
...
C ... Possibly add a special branch for the hermetian case
      if (.false.) then
      else
