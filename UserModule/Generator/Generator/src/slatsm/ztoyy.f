...
C      integer w(1),owk
C      common /w/ w
C      call defrr(owk,max(nr,nc))
C#endif

      if (kcplxi .eq. kcplxf) goto 100
      call isanrg(kcplxi,0,2,'ztoyy','kcplxi',.true.)
      call isanrg(kcplxf,0,2,'ztoyy','kcplxf',.true.)
...
C#elseC
C        call ztoy2(a,lda,ldb,nr,nc,w(owk),1)
C#endif
        kcplxl = 2
      endif
...
C#elseC
C        call ztoy2(a,lda,ldb,nr,nc,w(owk),0)
C#endif
        kcplxl = 0
      endif
