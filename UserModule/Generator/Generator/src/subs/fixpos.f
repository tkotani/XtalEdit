...
C Local parameters:
      ...
      double precision dbas(3),bast(3),sum,tol2,qlat(3,3),vol
C#ifdef F90 | AUTO-ARRAY
      double precision sdpos(3,nbas)
C#elseC
C      integer nbmx
C      parameter (nbmx=2000)
C      double precision sdpos(3,nbmx)
C      if (nbas .gt. nbmx) call rx('fixpos: increase nbmx')
C#endif
      ...
      do  10  ibas = 1, nbas
        do  20  ig = 1, ng
          ...
          do  30  m = 1, 3
   30     dbas(m) = bast(m) + ag(m,ig) - pos(m,jbas)
...
C     ... Debugging check          
  ...
  334     format(a,3f18.12)
   ...
   10 continue
      ...
      call awrit1(' FIXPOS: shifted site positions by average %g',' ',
     .  80,lgunit(1),sum/ng)
