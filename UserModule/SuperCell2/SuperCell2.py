...
if __name__ == '__main__':
	...
	#outxxx = outxxx + "Trans = ["											#Add T.sora
	#for i in range(len(Trans)):												#
	#	outxxx = outxxx + "%s\n" % (supersub.PVtoPVo(Trans[i]))	#
...
#	RasMinNum	=	0
	for nline in range(len(wtext)):
		...
		if string.find(s2,"SV")==0:
			...
			sss=string.replace(s,  'PV1','PV1o')  # check line include CV
			sss=string.replace(sss,'PV2','PV2o')  #
			sss=string.replace(sss,'PV3','PV3o')  #
		   	outdata = outdata + sss + "\n"        #
		for key in CheckVariable:
			...
			if	string.find(s2,key)==0:
				...
				if	PVKeyNum==6:
					...
					outdata = outdata + "CV1 = SV1n\n"  # add T.sora 2003/8/26
					outdata = outdata + "CV2 = SV2n\n"  # 
					outdata = outdata + "CV3 = SV3n\n"  #
					for s in Trans:
						outdata = outdata + s+"\n"
				break
		if flg==1:
