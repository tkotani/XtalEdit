...
def  vol(PV1,PV2,PV3):
	return  abs(determinant(array([PV1,PV2,PV3])))
...
def  perioP(PV1,PV2,PV3, POS):
	...
	invpvmat= inverse(pvmat)
	P= matrixmultiply(invpvmat,POS)
	P[0] = perio(P[0])
	P[1] = perio(P[1])
	P[2] = perio(P[2])
	return matrixmultiply(pvmat,P),P
...
def  SetNNN(PV1,PV2,PV3, SV1in,SV2in,SV3in):
	...
	for i in  range(Min[0]-0.1,Max[0]+1.1,1):
		for j in  range(Min[1]-0.1,Max[1]+1.1,1):
			for k in  range(Min[2]-0.1,Max[2]+1.1,1):
